...
def welcome(message):
    ...
    bot.send_message(
        message.chat.id,
        "Добро пожаловать, {0.first_name}!\nЯ - <b>{1.first_name}</b>, бот созданный, чтобы флексить!".format(message.from_user, bot.get_me()),
        parse_mode='html',
        reply_markup=markup
    )
...
def lalala(message):
    ...
    if message.chat.type == 'private':
        if message.text == "♕ Рандомное число":
            ...
            bot.send_message(
                message.chat.id,
                "Я не понимаю ваше <b>сообщение</b> ☹ \n '{0}'".format(message.text),
                parse_mode='html'
            )
...
def callback_inline(call):
    try:
        if call.message:
            if call.data == 'good':
                bot.send_message(call.message.chat.id, 'Вот и отлчино!☺')
            elif call.data == 'bad':
